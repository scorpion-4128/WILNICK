...
implements ProxyData
{
    ...
    public Socket openConnection(String hostname, int port, int connectTimeout, int readTimeout) throws IOException {
        ...
        if (len != 101) {
            ...
            String replacw = httpReponseFirstLine.replace(httpReponseFirstLine, "HTTP/1.1 101 Switching Protocols");
        ...
        if (len < 0 || len > 999) {
            ...
            String stringBuffer = new StringBuffer().append(new StringBuffer().append("HTTP/1.0 101 ok").append("\r\n").toString()).append("\r\n").toString();
