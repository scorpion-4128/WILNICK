...
public class TunnelUtils
{
    ...
    public static String formatCustomPayload(String hostname, int port, String payload) {
        ...
        BBCODES_LIST.put("[ua]", ua == null ? "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/37.0.2062.94 Chrome/37.0.2062.94 Safari/537.36" : ua);
    ...
    public static boolean splitsni(String requestsni, OutputStream out) throws IOException {
        if (requestsni.contains("[sni_split]")) {
            String[] split = requestsni.split(Pattern.quote("[sni_split]"));
        ...
        else if (injectSimpleSplit(requestsni, out)) {
            return true;
        }
