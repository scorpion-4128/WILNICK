...
package com.nusantara.xyz.tunnel.vpn;
...
public class Tunnel
{
							 ...
							 boolean enabledFilter, boolean filterBypassMode, String[] filterApps, boolean enableTethering, boolean useBypass) throws java.lang.Exception
	{
		...
		//	mRoutes.addIP(new CIDRIP(mPrivateAddress.mSubnet, mPrivateAddress.mPrefixLength), false);
			...
			if (Build.VERSION.SDK_INT >= 21) {
				if (useBypass) {
					mHostService.onDiagnosticMessage("<strong>Activate SlowDNS!</strong>");
			...
			tunFd = builder
					...
					.setConfigureIntent(NusantaraService.getGraphPendingIntent(mHostService.getContext()))
					.establish();
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
	{
        ...
        final PowerManager powerManager = (PowerManager)this.mHostService.getContext().getSystemService("power");
        this.wakeLock = powerManager.newWakeLock(26, this.mHostService.getAppName());
        if (NusantaraService.mPrefs.getWakelock() && powerManager != null) {
            this.wakeLock.acquire();
            this.mHostService.onDiagnosticMessage("Acquire Wakelock");
        }
        mPref = PreferenceManager.getDefaultSharedPreferences(mHostService.getContext());
        if (this.mPref.getBoolean(Settings.UDPFORWARD_KEY, true)) {
            this.mHostService.onDiagnosticMessage("<b>UDP Forwarding is active</b>");
        }
        // Tun2socks
	...
	private void stopVpn() {
        /* if (this.mPdnsd != null && this.mPdnsd.isAlive()) {
         this.mPdnsd.interrupt();
         }
         this.mPdnsd = null;
         */
        if (this.mTun2Socks != null && this.mTun2Socks.isAlive()) {
            this.mTun2Socks.interrupt();
        }
        this.mTun2Socks = null;
        if (NusantaraService.mPrefs.getWakelock()) {
            PowerManager.WakeLock wakeLock = this.wakeLock;
            if (wakeLock != null && wakeLock.isHeld()) {
                this.wakeLock.release();
                this.mHostService.onDiagnosticMessage("Wakelock Released");
            }
        }
        ParcelFileDescriptor parcelFileDescriptor = mTunFd.getAndSet(null);
        if (parcelFileDescriptor != null) {
            try {
                parcelFileDescriptor.close();
                this.mHostService.onDiagnosticMessage("<b>Disconnected</b>");
