...
public class SSLProxy implements ProxyData
{
    ...
    public SSLProxy(String server, int port, String sni, String requestsni) {
        this.stunnelServer = server;
        this.stunnelPort = port;
        this.stunnelHostSNI = sni;
        this.proxyUser = null;
        this.proxyPass = null;
    ...
    @Override
    public Socket openConnection(String sni, int port, int connectTimeout, int readTimeout) throws IOException
    {
        ...
        {
            mSocket = doSSLHandshake(sni, stunnelHostSNI, port);
            mSocket.setKeepAlive(true);
            mSocket.setTcpNoDelay(true);
            //SkStatus.logInfo(R.string.state_proxy_running);
            String requestsni = getRequestPayload(sni, port);

            OutputStream out = mSocket.getOutputStream();
            // suporte a [split] na payload
            if (!TunnelUtils.splitsni(requestsni, out)) {
                try {
                    out.write(requestsni.getBytes("ISO-8859-1"));
                } catch (UnsupportedEncodingException e2) {
                    out.write(requestsni.getBytes());
                }
                out.flush();
            }
    ...
    private String getRequestPayload(String sni, int port) {
        String payload = this.requestsni;

        if (payload != null) {
            payload = TunnelUtils.formatCustomPayload(sni, port, payload);
        }
        else {
